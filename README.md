...
This project/tutorial explores the Fast Gradient Sign Method (FGSM) attack and evaluates three different defense mechanisms aimed at removing these adversarial perturbations before classification to aid the classiifier:
...
This repository contains a fully runnable Jupyter Notebook designed for Google Colab (L4 GPU instance recommended). There are no prerequisites to run the code, it is fully self contained.
...
1) Implement an FGSM attack generator to fool a target classifier (A CNN in this case).

2) Develop three distinct image preprocessing defenses.

3) Evaluate the robustness of each defense using the following quantitative metrics
 - Accuracy
 - F1 Score (Macro)
 - Recall
 - Precision
 - Recovery Rate (Detailed in 
...
Open the Computer_Vision_Project.ipynb in Google Colab.

Go to Runtime -> Change runtime type.

Select GPU as the hardware accelerator (an L4 GPU was used for development and is highly recommended for training the Autoencoders).
...
* **Sabotage Level ($\epsilon$):** Represents the magnitude of the adversarial perturbation applied to the clean image. It is the maximum change allowed for any individual pixel, where $\epsilon = 0.0$ representas a clean baseline and higher values represent more aggressive perturbations.
...
Run the Imports and Setup cells to install requirements (tensorflow, opencv-python, etc.).

Run the Data Loading cells (the dataset will download automatically via kagglehub).

Train or load the Target Classifier.

Train the Standard Autoencoder and U-Net Autoencoder on the clean training set.

Execute the Evaluation Loop, which generates FGSM samples on the test set, passes them through the three defenses, and records the metrics.
